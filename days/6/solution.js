module.exports = function (input) {
    ...
    const low = (time, distance) => Math.ceil((-time + (time**2 - 4*distance)**0.5)/(-2))
    for (let i = 0; i < times.length; i++) {
        part1 *= high(times[i], distances[i]) - low(times[i], distances[i]);
    }
    const time = parseInt(lines[0].split(/\s+/).slice(1).join(''))
    const distance = parseInt(lines[1].split(/\s+/).slice(1).join(''))
    const part2 = high(time, distance) - low(time, distance);
    return [part1, part2];
}
